...
2. Create a new Webhook for your application and point it to: `https://plateit-api.co.uk/v3/webhooks/paypal` ensuring the event types `PAYMENT.CAPTURE.COMPLETED` and `PAYMENT.CAPTURE.REFUNDED` are selected.
...
PayPal payments can take an `invoice_id` parameter. **Ensure this is populated with the pending order ID returned from the [place-order](/actions/place-order.md) endpoint.**
...
The payment will be logged accordingly and the status of the order will be updated from `pending` to `open`.
